...
以上の環境のもと、各アルゴリズムで $N = 10^{\protect\input{n-samples.tex}}$ 個のサンプリングを行いました。表 \ref{tab:benchmark-result} に計測結果をまとめます。また、図 \ref{fig:histogram} はそれぞれのアルゴリズムで生成した乱数列のヒストグラムです。

\begin{table}
    \caption{ベンチマーク結果 ($N = 10^{\protect\input{n-samples.tex}}$)}
...
\begin{figure}[t]
    ...
    \end{subfigmatrix}
    \caption{生成した乱数列のヒストグラム ($N = 10^{\protect\input{n-samples.tex}}$)}
    \label{fig:histogram}
\end{figure}
